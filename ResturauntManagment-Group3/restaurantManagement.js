...
async function getDbConnection() {
  db = await mysql.createConnection({
    host: 'localhost',const http = require('http');
const url = require('url');
const mysql = require('mysql2/promise');
const bcrypt = require('bcrypt');
    ...
    try {
      ...
      */
      ...
      if (password){
        ...
        res.end(JSON.stringify({ message: "Password required to register a new restaurant" }));
        return;
      }
      const query = `INSERT INTO Restaurant (${queryValues.join(', ')}) VALUES (${Array(queryParams.length).fill('?').join(', ')})`;
...
async function editRestaurant(req, res, restaurantId) {
  ...
  req.on('end', async () => {
    ...
    try {
      ...
      const restaurantDetails = properties[0];
      const password = req.headers['x-password'];
